...
if guiparent:FindFirstChild("CKPV") then
    guiparent:FindFirstChild("CKPV"):Destroy()
end
...
-- ScrollingFrame แทน contentFrame
...
contentFrame.ScrollBarThickness = 0 -- ซ่อน scrollbar
...
local function updateCanvas()
	local totalSize = layout.AbsoluteContentSize.Y
	contentFrame.CanvasSize = UDim2.new(0, 0, 0, totalSize + layout.Padding.Offset)
end
layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateCanvas)

-- Reorder function
function GUI:Reorder()
	local order = 0
	for _, data in ipairs(GUI.Elements) do
...
-- Header
function GUI:AddHeader(text, width, height)
	...
	table.insert(GUI.Elements, {Frame = label})
	GUI:Reorder()
	return label
end

-- Toggle
function GUI:AddToggle(name, text, default, callback)
	...
	GUI.Elements[#GUI.Elements+1] = {Frame = frame, Type = "Toggle", Name = name, Object = toggle}
	GUI:Reorder()
	return toggle
end

-- Button
function GUI:AddButton(name, text, callback)
...
function GUI:AddDropdown(name, text, default, callback)
	local btn = Instance.new("TextButton")
	btn.Size = UDim2.new(1, 0, 0, 36)
